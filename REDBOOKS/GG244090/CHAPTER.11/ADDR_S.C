...
/*           module addr_m.c. This module registers manager EPV,              */
...
#define ENTRY_NAME "/.:/Servers/Addr"   /* Server entry name.                 */

extern look_v1_0_epv_t epv_addr;        /* Address manager EPV table.         */
...
int main ( int argc, char *argv[] )
{
   ...
   uuid_t obj_uuid_addr, type_uuid_addr;
   ...
   uuid_from_string ( ADDR_OBJ_UUID, &obj_uuid_addr, &status );
   ERRCHK ( status );

   /* Initialize the type UUID.                                               */
   uuid_create ( &type_uuid_addr, &status );

   /* Associate object UUID with type UUID.                                   */
   rpc_object_set_type ( &obj_uuid_addr, &type_uuid_addr, &status );
   ERRCHK ( status );

   /* Register interface/epv associations with RPC runtime.                   */
   printf("Registering server interface with RPC runtime...\n");
   rpc_server_register_if ( look_v1_0_s_ifspec, &type_uuid_addr,
                            ( rpc_mgr_epv_t )&epv_addr, &status );
   ERRCHK ( status );
   ...
   obj_uuid_vec.vec[ 0 ] = &obj_uuid_addr;
   ...
   rpc_ep_register ( look_v1_0_s_ifspec, bv_p,
                     ...
                     ( unsigned_char_t * )"Address server, version 1.0",
                     &status );
   ERRCHK ( status );
