...
static char sccs_id[] = "@(#)Subrectangle of General Bitmap  1/1/95";
...
int main(int argc, char *argv[])
	...
	if ( w     == 0     ) { close(fd); fatal("w = 0"); }
	if ( h     == 0     ) { close(fd); fatal("h = 0"); }
	if ( x     >= gbm.w ) { close(fd); fatal("x >= bitmap width"); }
	if ( y     >= gbm.h ) { close(fd); fatal("y >= bitmap height"); }
	if ( x + w >  gbm.w ) { close(fd); fatal("x+w > bitmap width"); }
	if ( y + h >  gbm.h ) { close(fd); fatal("y+h > bitmap height"); }
