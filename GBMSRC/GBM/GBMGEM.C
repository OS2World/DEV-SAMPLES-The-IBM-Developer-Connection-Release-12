...
GBM_ERR gem_rhdr(const char *fn, int fd, GBM *gbm, const char *opt)
	...
	lseek(fd, priv->lenhdr*2, SEEK_SET);
...
lseek(fd, (8+3)*2, SEEK_SET);
for ( i = 0; i < ncols; i++ )
	{
	byte palentry[3*2];
	if ( read(fd, palentry, 3*2) != 3*2 )
		return GBM_ERR_READ;
	...
	for ( y = gbm->h - 1; y >= 0; y--, data -= stride )
		if ( vrep )
			...
			for ( p = 0, bit = 0x11; p < priv->planes; p++, bit <<= 1 )
				...
				if ( bits )
					spread((byte) (line[x] & (0xff00 >> bits)), bit, dest);
				}
			}
	}
	break;
...
case 24:
	...
	for ( y = gbm->h - 1; y >= 0; y--, data -= stride )
		if ( vrep )
			...
			{
			read_gem_line(ahead, data, scan*24, priv->patlen, &vrep, TRUE);
...
case 1:
	...
	for ( y = gbm->h; y > 0; y -= dy, data -= dy*stride )
		...
		if ( write(fd, enc, len_enc) != len_enc )
			{ free(enc); free(line); return GBM_ERR_WRITE; }
		}
...
case 4:
	...
	if ( pal )
		...
		if ( write(fd, hdr, 9*2) != 9*2 )
			return GBM_ERR_WRITE;
		for ( i = 0; i < 0x100; i++ )
			map[i] = (map_to_grey4(gbmrgb+((i>>4)&15))<<4) |
				  map_to_grey4(gbmrgb+( i    &15))     ;
		}
	else
		{
		int i;
		if ( write(fd, hdr, 8*2) != 8*2 )
			return GBM_ERR_WRITE;
		for ( i = 0; i < 0x100; i++ )
			map[i] = (map_to_col4(gbmrgb+((i>>4)&15))<<4) |
				  map_to_col4(gbmrgb+( i    &15))     ;
		}

	if ( (mapped = malloc((gbm->w+1)/2)) == NULL )
		return GBM_ERR_MEM;
...
case 24:
	...
	if ( (line = malloc(gbm->w*3)) == NULL )
		return GBM_ERR_MEM;

	if ( (enc = malloc(gbm->w*3*2+2)) == NULL )
		{ free(line); return GBM_ERR_MEM; }
