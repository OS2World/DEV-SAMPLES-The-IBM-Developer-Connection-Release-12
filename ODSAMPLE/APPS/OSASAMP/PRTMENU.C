...
MRESULT Menu( HWND hwnd, ULONG msg, MPARAM mp1, MPARAM mp2 )
{
   ...
   char Filename[20]= "mf.cmd";  /*dq42595  */ 
   AEDesc aeDescrRec;   /* the descriptor record containing the name */  /*dq42595  */ 
   OSErr    rctest = noErr;  /* dq42595 */
   Size dataSize = 0;  /* dq42595 */
   DescType typeCode; /* dq42595 */ 
   char dataBuffer[20] = "nothing";  /* dq42595 */ 
   Size actualSize = 0 ; /* dq42595 */  
   Size  maximumSize = 20;  /* dq42595 */  
   ...
   case WM_COMMAND:
      ...
      case IDM_OPEN:
         ...
         */
#ifdef OSA_AWARE
        /* Generate a Apple Event - Open Document */
        rc =  GenerateOSAEvent(hwnd, msg, MPFROMSHORT(IDM_AEOPENDOC), mp2);
#else
         WinFileDlg( HWND_DESKTOP, hwnd, &pmp->filedlg );
         return ( (MRESULT)ValidateFilename( pmp, pmp->szNextFilename, hwnd) );  /* dq0927 NEW */
#endif
         return (MRESULT)NULL;
      ...
      case IDM_PRINT:
#ifdef OSA_AWARE
        /* Generate a Apple Event - Print Document */
        rc =  GenerateOSAEvent(hwnd, msg, MPFROMSHORT(IDM_AEPRINTDOC), mp2);
#else
         Print(hwnd, pmp);
#endif
         return (MRESULT)NULL;
...
/* 
dq0927  Change this procedure to not call ValidateFilename anymore
dq0927  so that is is more generalized and able to be called by new 
dq0927  Apple Event Generator code to get the list of filenames to open
dq0927  (old comment was   validate filename and use if ok )
dq0927      return ( (MRESULT)ValidateFilename( pmp, (PSZ)mp1, hwnd) );
*/
     /* Save new file name in global data for later use */
     strcpy( pmp->szNextFilename, (PSZ)mp1 ); /* dq0927 */
      break;
