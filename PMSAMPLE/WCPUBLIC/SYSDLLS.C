...
void cdecl main() {
   ...
   rc=DosAllocSeg(((USHORT)Length)+strlen(" TITLEBAR")+1,&Sel,SEG_NONSHARED);
   rc=DosAllocSeg(((USHORT)Length)+strlen(" TITLEBAR")+1,&Sel1,SEG_NONSHARED);
   pBuffer=MAKEP(Sel,0);
   ...
   if (Found==FALSE) {
      strcat(pBuffer1," TITLEBAR");
      printf("Adding TITLEBAR\n");
   } /* endif */
